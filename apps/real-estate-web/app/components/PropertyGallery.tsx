      ...
      <div className="relative">
        <div className="relative h-80 w-full bg-zinc-900">
          {current.video_url ? (
            ...
            <Image src={current.url} alt={current.caption || 'Property image'} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
          ) : null}
        </div>
      ...
      <div className="flex gap-2 overflow-x-auto">
        {images.map((img, i) => (
          <button key={img.id} onClick={() => setIndex(i)} className={`rounded-md overflow-hidden ${i===index ? 'ring-2 ring-amber-400' : ''}`}>
            {img.video_url ? <video src={img.video_url} muted className="h-16 w-28 object-cover" /> : img.url ? <Image src={img.url} alt={img.caption || 'thumb'} width={112} height={64} className="h-16 w-28 object-cover" /> : null}
          </button>
        ))}
      </div>
    </div>
