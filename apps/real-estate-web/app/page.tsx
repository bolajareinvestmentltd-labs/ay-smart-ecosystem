  ...
  return (
    <main className="min-h-screen bg-[var(--brand-surface)] pb-28 text-[var(--text-primary)]">
      <div className="mx-auto max-w-6xl px-4 pb-8 sm:px-6 lg:px-8">
        ...
        <section className="mt-6"><div className="flex items-center justify-between"><h2 className="text-lg font-black tracking-[-0.04em]">Popular Hostels</h2><Link href="/hostel" className="text-xs font-bold text-[#4e235f]">See all</Link></div><div className="mt-3 space-y-2">{listings.filter((listing) => listing.category === "Hostel").slice(0, 2).map((listing) => <Link href={`/hostel/${listing.id}`} key={listing.id} className="flex items-center gap-3 rounded-2xl border border-[#eaded9] bg-white p-2 shadow-[0_6px_18px_rgba(78,35,95,0.05)]"><Image src={listingImage(listing) || "/assets/ay-smart-logo.png"} alt={listing.title} width={80} height={64} className="h-16 w-20 rounded-xl object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{listing.title}</p><p className="mt-1 text-[11px] text-[#817681]">{listing.location} · <Star size={10} fill="#f1a990" className="inline text-[#e28c72]" /> Live</p></div><p className="text-xs font-black text-[#4e235f]">₦{Number(listing.price || 0).toLocaleString()}</p></Link>)}</div></section>
