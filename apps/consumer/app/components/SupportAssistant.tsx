...
export default function SupportAssistant() {
  ...
  async function ask(event: React.FormEvent) {
    ...
    try {
      ...
      setMessages((current) => [...current, { from: 'bot', text: answer, escalate: Boolean(payload.fallback) || !response.ok }]);
    } catch {
      setMessages((current) => [...current, { from: 'bot', text: 'The assistant is temporarily unavailable. Please use Contact support for a human response.', escalate: true }]);
    } finally {
      setAsking(false);
    }
  }
  ...
  return (
    <>
      ...
      {open && <section className="fixed bottom-40 right-4 z-40 flex w-[min(22rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-[var(--brand-border)] bg-[var(--brand-surface-2)] shadow-2xl sm:bottom-20 sm:right-5" aria-label="Support assistant">
        <div className="flex items-start justify-between bg-[#4e235f] p-4 text-white"><div><p className="font-bold">AY&apos;SMART assistant</p><p className="text-xs text-white/75">AI onboarding and support help</p></div><button type="button" onClick={() => setOpen(false)} aria-label="Close support assistant" className="rounded-full p-1 hover:bg-white/10"><X size={16} /></button></div>
        <div className="max-h-64 space-y-2 overflow-y-auto p-3 text-sm">{messages.map((message, index) => <div key={`${message.from}-${index}`} className={`rounded-xl p-2 ${message.from === 'user' ? 'ml-8 bg-[#f1b8a5]/30 text-[var(--text-primary)]' : 'mr-4 bg-[var(--brand-surface-3)] text-[var(--text-primary)]'}`}><p>{message.text}</p>{message.escalate && <Link href="/support" className="mt-2 inline-block text-xs font-bold text-[#4e235f] underline">Contact support</Link>}</div>)}</div>
