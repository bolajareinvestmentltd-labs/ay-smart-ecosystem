...
export async function loginWithGoogle(tokenOrCredential: string, email?: string, name?: string) {
  try {
    await ensureCsrfCookie();
    const res = await fetch(buildApiUrl('/api/auth/google/'), {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ credential: tokenOrCredential, token: tokenOrCredential, email, name }),
    });
    const payload = await res.json().catch(() => ({}));
    return { ok: res.ok, payload };
  } catch {
    return { ok: false, payload: { detail: 'Network error during Google sign-in.' } };
  }
}
